...
def mainLoopFunc(user_input):
    ...
    def serverPort():
        ...
        ######
        # THIS OPENS A PORT
        mainLoopFunc.command_input = input(yellow2 + " What port do you want the server to listen on?\n")
        mainLoopFunc.serverPort = mainLoopFunc.command_input
        subprocess.Popen(['./ServerRequirements/eguiHTTPS.py', mainLoopFunc.serverPort], shell=False)
        print(green3 + " Server is now listening on " + colorGreen + mainLoopFunc.serverPort + colorBlack)
        # END OPEN PORT
        #######
        # Appends the opened port to the global list serverPortList
        serverPortList.append(mainLoopFunc.serverPort)
        #print("Server Port List is NOW: " + str(serverPortList))
