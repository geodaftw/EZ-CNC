...
def mainLoopFunc(user_input):
    ...
    def serverPort():
        ...
        if mainLoopFunc.command_input == 'add':
            ######
            # THIS OPENS A PORT
            # Try/Else to make sure its an integer
            try:
                addPort = int(input(yellow2 + " What port do you want the server to listen on?\n")) # changed mainLoopFunc.command_input to "addPort"
            except ValueError:
                print(red4 + " That's not a number! Starting over..")
            else:
                mainLoopFunc.serverPort = str(addPort)
                ...
                print(green2 + " Adding Port " + colorBlue + mainLoopFunc.serverPort + colorBlack)
            ...
            try:
                Port = int(input(yellow2 + " Which port do you want to remove?\n"))
            except ValueError:
                print(red4 + " That's not a number! Starting over..")
            else:
                removePort = str(Port)
                # Loop to find a match of server port in psutil. When found, run proc.kill()
                if any(item.lower() == removePort.lower() for item in serverPortList):
                    serverPortList.remove(str(removePort))
                    PROCNAME = "eguiHTTPS.py"
                    for proc in psutil.process_iter():
                        # This Try/Except is needed due to issues with IndexError.
                        try:
                            if proc.name() in PROCNAME and proc.cmdline()[2] == str(removePort):
                                print(red4 + ' Closing Port ' + colorBlue + str(proc.cmdline()[2]) + colorBlack)
                                proc.kill()
                        except IndexError:
                            pass
                else:
                    print(red1 + " Didn't match")
        else:
            print(red1 + " Not a valid option")
...
if __name__ == "__main__":
    ...
    while True:
        ...
        # Run the mainLoopFunc based off user_input
        mainLoopFunc(user_input) 
        # Update user_input to lastUser_input
        lastUser_input = user_input
        # Update the command_input to last command input
        lastCommand_input = mainLoopFunc.command_input
        # Update serverPort to additional ServerPort
        additionalPort = mainLoopFunc.serverPort
        
        _ = os.system('clear')
    
    # Run the cleanup Function once the loop breaks and gets the quit function
    cleanupFunc()
